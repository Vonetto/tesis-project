...
def scan_bronze_dataset(dataset_name: str) -> pl.LazyFrame:
    ...
    try:
        if USE_LOCAL_PATHS:
            ...
            # For GCS, use storage_options
            return pl.scan_parquet(f"{path}/**/*.parquet", storage_options={
...
def read_csv_portable(path: str, sep=";") -> pl.DataFrame:
    if USE_LOCAL_PATHS:
...
df_zonas_777 = read_csv_portable(f"{RAW_PATH}/DIC_777.csv")
...
def read_shapefile_portable(shape_folder_path: str) -> gpd.GeoDataFrame:
    """Descarga un shapefile de GCS a un directorio temporal y lo lee con pyogrio, o lo lee directamente si es local."""
    if USE_LOCAL_PATHS:
        # For local paths, assume shapefile is directly accessible
        shp_local = Path(shape_folder_path) / "Zonas777.shp" # Assuming a fixed name
        if not shp_local.exists():
            raise FileNotFoundError(f"No se encontró archivo .shp en {shape_folder_path}")
        gdf = pyogrio.read_dataframe(str(shp_local))
        if gdf.crs is None:
...
if USE_LOCAL_PATHS:
    ...
    gdf_zonas777_geo.to_parquet(SILVER_GEOMETRIA_ZONAS_PATH, pyarrow_options={"filesystem": gfs})
...
try:
    if USE_LOCAL_PATHS:
        # Volvemos a usar el glob `/**/*.parquet` para ser explícitos y evitar
        # archivos ocultos de macOS (._) que rompen el descubrimiento de particiones.
        lf_silver_check = pl.scan_parquet(f"{SILVER_VIAJES_PATH}/**/*.parquet")
    else:
        # Para GCS, el descubrimiento automático suele ser seguro, pero usamos glob para consistencia.
        lf_silver_check = pl.scan_parquet(f"{SILVER_VIAJES_PATH}/**/*.parquet", storage_options={
            "google_cloud_storage": True,
            "service_account": os.getenv("GOOGLE_APPLICATION_CREDENTIALS")
        })
