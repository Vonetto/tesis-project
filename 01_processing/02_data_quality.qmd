...
try:
    ...
    full_glob_path = f"{particion_path}*.parquet"
    print(f"📁 Buscando archivos en: {full_glob_path}")
    
    # Usar read_parquet_portable para cargar el LazyFrame
    df_lazy = read_parquet_portable(full_glob_path)
    print("✅ LazyFrame creado exitosamente")
...
if 'df_dist' in locals() and df_dist is not None:
    ...
    try:
        ...
        if USE_LOCAL_PATHS:
            ...
            # We still need to ensure the directory exists for GCS
            fs = get_filesystem()
            fs.makedirs(output_partition.replace("gs://", "").strip("/"), exist_ok=True)
            
        df_para_guardar.sink_parquet(output_file, compression='snappy')
        ...
        for year_dir in pathlib.Path(INPUT_PATH).glob("iso_year=*"):
            ...
            for week_dir in year_dir.glob("iso_week=*"):
                ...
                particiones.append({
                    'year': year,
                    'week': week,
                    'path': str(week_dir / "data-0.parquet") # Assuming fixed filename
                })
    else:
        fs = get_filesystem()
        for year_dir in fs.glob(f"{INPUT_PATH.replace('gs://', '').strip('/')}/iso_year=*"):
            ...
            for week_dir in fs.glob(f"{year_dir}/iso_week=*"):
                ...
                particiones.append({
                    'year': year,
                    'week': week,
                    'path': f"gs://{week_dir}/data-0.parquet"
                })
...
if particiones:
    ...
    for particion in tqdm(particiones_pendientes, desc="Procesando particiones"):
        ...
        try:
            ...
            # Leer partición
            df = read_parquet_portable(input_file).collect()
...
if USE_LOCAL_PATHS:
    for year_dir in pathlib.Path(INPUT_PATH).glob("iso_year=*"):
        ...
        for week_dir in year_dir.glob("iso_week=*"):
            week = int(str(week_dir).split('iso_week=')[1])
            if (week_dir / "data-0.parquet").exists():
    ...
    for year_dir in fs.glob(f"{INPUT_PATH.replace('gs://', '').strip('/')}/iso_year=*"):
        ...
        for week_dir in fs.glob(f"{year_dir}/iso_week=*"):
            week = int(week_dir.split('iso_week=')[1])
            if fs.exists(f"{week_dir}/data-0.parquet"):
                input_partitions.add((year, week))
...
if USE_LOCAL_PATHS:
    if pathlib.Path(OUTPUT_PATH).exists():
        for year_dir in pathlib.Path(OUTPUT_PATH).glob("iso_year=*"):
            year = int(str(year_dir).split('iso_year=')[1])
            for week_dir in year_dir.glob("iso_week=*"):
                week = int(str(week_dir).split('iso_week=')[1])
                if (week_dir / "data-0.parquet").exists():
    ...
    fs = get_filesystem()
    if fs.exists(OUTPUT_PATH.replace("gs://", "").strip("/")):
        for year_dir in fs.glob(f"{OUTPUT_PATH.replace('gs://', '').strip('/')}/iso_year=*"):
            year = int(year_dir.split('iso_year=')[1])
            for week_dir in fs.glob(f"{year_dir}/iso_week=*"):
                week = int(week_dir.split('iso_week=')[1])
                if fs.exists(f"{week_dir}/data-0.parquet"):
