...
try:
    ...
    print("🔍 Buscando archivos .parquet en Silver...")
    
    if USE_LOCAL_PATHS:
        archivos_parquet = [str(p) for p in pathlib.Path(INPUT_PATH).glob("**/*.parquet")]
    else:
        archivos_parquet = gfs.glob(f"{INPUT_PATH.replace('gs://', '').strip('/')}/**/*.parquet")
        archivos_parquet = [f"gs://{p}" for p in archivos_parquet] # Add gs:// scheme back for Polars
    
    print(f"   ✅ Encontrados {len(archivos_parquet)} archivos")
    ...
    print("🔄 Convirtiendo a Polars LazyFrame...")
    if USE_LOCAL_PATHS:
        lf_viajes = pl.scan_parquet(archivos_parquet)
    else:
        # Crear dataset de PyArrow usando el filesystem autenticado
        # partitioning=None evita problemas con tipos inconsistentes
        print("📖 Configurando dataset con PyArrow...")
        dataset = ds.dataset(
            [p.replace("gs://", "").strip("/") for p in archivos_parquet],  # Lista de rutas sin gs://
            filesystem=fs_arrow,  # Usa el filesystem autenticado
            format="parquet",
            partitioning=None  # No leer columnas de partición (evita conflictos de tipos)
        )
...
if 'validos_globales' in globals() and 'total_viajes' in globals():
    ...
    try:
        ...
        if USE_LOCAL_PATHS:
            lf_filtrados_real = pl.scan_parquet(f"{OUTPUT_FILTRADOS_PATH}/**/*.parquet")
        else:
            lf_filtrados_real = pl.scan_pyarrow_dataset(
                ds.dataset(OUTPUT_FILTRADOS_PATH, filesystem=fs_arrow, format="parquet")
            )
