...
if USE_LOCAL_PATHS:
    ...
    gfs = None # Not needed for local paths
    print(f"Buscando datos en: {RAW_BASE_PATH}/ (Local)")
else:
    RAW_BASE_PATH = f"gs://{GCS_BUCKET_NAME}/{GCS_RAW_PREFIX}"
    BRONZE_BASE_PATH = f"gs://{GCS_BUCKET_NAME}/{GCS_BRONZE_PREFIX}"
    gfs = gcsfs.GCSFileSystem(token="google_default")
    print(f"Buscando datos en: {RAW_BASE_PATH}/ (GCS)")
...
try:
    ...
    if USE_LOCAL_PATHS:
        ...
        local_shp = pathlib.Path(folder) / "Zonas777.shp" # Assuming a fixed name
        if not local_shp.exists():
            raise FileNotFoundError(f"No se encontró el shapefile local en {local_shp}")
        try:
            import pyogrio
            gdf_zonas = pyogrio.read_dataframe(str(local_shp))
        except Exception:
            gdf_zonas = gpd.read_file(str(local_shp))
        if getattr(gdf_zonas, "crs", None) is None:
            gdf_zonas = gdf_zonas.set_crs(4326, allow_override=True)
        print(f"GeoDataFrame 'gdf_zonas' cargado exitosamente ({len(gdf_zonas)} zonas) desde local.")
    else:
        folder = f"{RAW_BASE_PATH}/Zonas777-2014/Shape"
        paths = gfs.ls(folder)
        ...
        if getattr(gdf_zonas, "crs", None) is None:
            gdf_zonas = gdf_zonas.set_crs(4326, allow_override=True)
        print(f"GeoDataFrame 'gdf_zonas' cargado exitosamente ({len(gdf_zonas)} zonas) desde GCS.")
...
if USE_LOCAL_PATHS:
    ...
    try:
        df_caracterizacion = pl.read_parquet(path_caracterizacion_parquet)
        print(f"Datos de caracterización cargados ({df_caracterizacion.height:,} filas) desde local.")
    except Exception as e:
        print(f"Error al cargar datos localmente: {e}")
        df_caracterizacion = None
else:
    path_caracterizacion_parquet = f"gs://{GCS_BUCKET_NAME}/{GCS_BRONZE_PREFIX}/caracterizacion/snapshot_date=2025-09-05/part-0.parquet"
    try:
        with gfs.open(path_caracterizacion_parquet, "rb") as f:
            df_caracterizacion = pl.read_parquet(f)
        print(f"Datos de caracterización cargados ({df_caracterizacion.height:,} filas) desde GCS.")
    except Exception as e:
        print(f"Error al cargar datos desde GCS: {e}")
        df_caracterizacion = None
