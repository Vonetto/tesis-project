...
from config.constants import USE_LOCAL_PATHS, LOCAL_BRONZE_PATH, GCS_BRONZE_PATH
...
def scan_bronze(dataset: str, semana_iso: str | None = None) -> pl.LazyFrame:
    base_path = f"{BRONZE_BASE_PATH}/{dataset}"
    if semana_iso:
        part_norm  = f"{base_path}/semana_iso={semana_iso}/*.parquet"
        return read_parquet_portable(part_norm)
    else:
        # If no specific week, scan the entire dataset
        return read_parquet_portable(f"{base_path}/**/*.parquet")
