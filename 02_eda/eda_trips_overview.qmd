...
from config.constants import GCS_BUCKET_NAME, GCS_BRONZE_PREFIX, GCS_RAW_PATH, USE_LOCAL_PATHS, LOCAL_BRONZE_PATH, LOCAL_RAW_PATH
...
try: 
    ...
    params = { 
        ...
        "gcs_bucket": os.getenv("GCS_BUCKET", GCS_BUCKET_NAME),
        "bronze_prefix": os.getenv("BRONZE_PREFIX", GCS_BRONZE_PREFIX),
...
# Asegurar que el paquete 'config' y 'lib' sean importables (si no se hizo en la celda anterior)
# Se busca el directorio raíz del proyecto (que contiene 'config/' y 'lib/') y se añade a sys.path
PROJECT_ROOT = Path().resolve()
while PROJECT_ROOT != PROJECT_ROOT.parent:
    if (PROJECT_ROOT / "config").is_dir() and (PROJECT_ROOT / "lib").is_dir():
...
# Importar constantes desde config
from config.constants import GCS_BUCKET_NAME, GCS_BRONZE_PREFIX, GCS_RAW_PATH, USE_LOCAL_PATHS, LOCAL_BRONZE_PATH, GCS_BRONZE_PATH
...
```{python}
lf_v = read_parquet_portable(f"{BRONZE_BASE_PATH}/{DATASET}/semana_iso={SEM_LABEL}/*.parquet")
...
# import pyogrio # Will be used internally by read_shapefile_portable when implemented
import shapely  # noqa: F401
import tempfile
from pathlib import Path

# HAS_GEO = True # Assume geopandas is always available if it was imported correctly

# Helpers mínimos
def _detect_sep_portable(path: str, default=";"):
    """
    Detects separator for CSV files. This helper is kept local for now,
    but could be moved to lib.datalake if needed elsewhere.
    """
    if USE_LOCAL_PATHS:
...
df_dic = None
try:
    dic_path = f"{RAW_BASE_PATH}/DIC_777.csv"
    sep_dic = _detect_sep_portable(dic_path)
    df_dic = read_csv_portable(
        dic_path,
        separator=sep_dic,
...
gdf_zonas = None
# if HAS_GEO: # Assuming geopandas is always available
try:
    # NOTA: read_shapefile_portable no está implementada aún en lib/datalake.py
    # Comentar su uso por ahora.
    # folder = f"{RAW_BASE_PATH}/Zonas777-2014/Shape"
    # gdf_zonas = read_shapefile_portable(folder)
    print("Carga de GeoDataFrame Zonas777 (gdf_zonas) omitida por ahora, ya que read_shapefile_portable no está implementada.")
except Exception as e:
    print("No se pudo cargar shapefile Zonas777:", e)

# 3) Construir puntos de paraderos (si hay x,y en DIC)
gdf_par = None
if df_dic is not None and {"x","y"} <= set(df_dic.columns):
    ...
    # Ensure geopandas is imported if needed for this part
    if 'gpd' not in sys.modules:
        import geopandas as gpd
...
if gdf_par is not None and "paradero_inicio_viaje" in lfk.collect_schema().names():
