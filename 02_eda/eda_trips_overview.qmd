...
# Importar constantes desde config
from config.constants import GCS_BUCKET_NAME, GCS_BRONZE_PREFIX, GCS_RAW_PATH
...
def scan_bronze_week(dataset: str, semana_iso: str) -> pl.LazyFrame:
    base = f"{BRONZE_BASE_PATH}/{dataset}"
    # Layout correcto
    ...
    try:
        if USE_LOCAL_PATHS:
            ...
            return pl.scan_parquet(path_norm, storage_options={
        ...
        try:
            if USE_LOCAL_PATHS:
                ...
                return pl.scan_parquet(path_tuple, storage_options={
                    "google_cloud_storage": True,
                    "service_account": os.getenv("GOOGLE_APPLICATION_CREDENTIALS")
                })
        except Exception as e:
            ...
            try:
                if USE_LOCAL_PATHS:
                    print("Particiones disponibles (local):", [str(p) for p in pathlib.Path(f"{BRONZE_BASE_PATH}/{dataset}").glob("iso_year=*")][:20])
                else:
                    print("Particiones disponibles (GCS):", gfs.ls(f"{BRONZE_BASE_PATH.replace('gs://', '').strip('/')}/{dataset}")[:20])
            except Exception:
                pass
            raise e
...
try:
    ...
    if USE_LOCAL_PATHS:
        ...
        with gfs.open(dic_path.replace("gs://", "").strip("/"), "rb") as fh:
            df_dic = pl.read_csv(
                fh,
                separator=sep_dic,
                infer_schema_length=20000,
...
if HAS_GEO:
    try:
        ...
        if USE_LOCAL_PATHS:
            local_shp = pathlib.Path(folder) / "Zonas777.shp"
            ...
            print("Zonas777 (local):", gdf_zonas.shape)
        else:
            paths = gfs.ls(folder)
            shp_remote = next(p for p in paths if p.lower().endswith(".shp"))
            if not shp_remote:
                raise FileNotFoundError(f"No se encontró archivo .shp en {folder}")
            ...
            if getattr(gdf_zonas, "crs", None) is None:
                gdf_zonas = gdf_zonas.set_crs(4326, allow_override=True)
            print("Zonas777 (GCS):", gdf_zonas.shape)
    except Exception as e:
