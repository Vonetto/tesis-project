...
def _ingest_viajes_files(file_paths: list[str]):
    ...
    for sem, dfs in buckets.items():
        ...
        # Derivar is_qr si hay columna de contrato en el DF
        if "id_contrato" in dfw.columns or "contrato" in dfw.columns:
            colc = "id_contrato" if "id_contrato" in dfw.columns else "contrato"
            dfw = dfw.with_columns(
                pl.col(colc).cast(pl.Utf8, strict=False).str.strip_chars().is_in(["171","102"]).alias("is_qr")
            )
...
def _ingest_etapas_files(file_paths: list[str]):
    ...
    for sem, dfs in buckets.items():
        ...
        dfw = pl.concat(dfs_aligned, how="vertical_relaxed").rechunk()
        dfw, sem_norm = _sanitize_before_write(dfw, sem)
        # Derivar is_qr si etapas aportan contrato (poco común); protegido
        if "id_contrato" in dfw.columns or "contrato" in dfw.columns:
            colc = "id_contrato" if "id_contrato" in dfw.columns else "contrato"
            dfw = dfw.with_columns(
                pl.col(colc).cast(pl.Utf8, strict=False).str.strip_chars().is_in(["171","102"]).alias("is_qr")
...
print(lf_v.columns)
print(lf_e.columns)
print(df_zonas_777.columns)
print(gdf_zonas777.columns)
print(gdf_par_4326.columns)
```