...
if "is_qr" in lfk.collect_schema().names():
    ...
    qr_sum = qr_sum.with_columns((pl.col("n") / pl.col("n").sum()).alias("pct_qr"))
...
if {"id_tarjeta", "is_qr"} <= set(lfk.collect_schema().names()):
    ...
    print("Perfil base por tarjeta (muestra):")
    print(by_card.head())
    ...
    ax_b.set_title("B: Nivel de Adopción QR por Tarjeta")
    ...
    # Creamos segmentos basados en la distribución observada
    def assign_segment(n):
        if n <= 5: return "1. Ocasional (1-5)"
        if n <= 15: return "2. Regular (6-15)"
        if n <= 30: return "3. Frecuente (16-30)"
        return "4. Intensivo (31+)"

    by_card_seg = by_card.with_columns(
        pl.col("n_trips").map_elements(assign_segment, return_dtype=pl.String).alias("segmento_uso")
    )
    ...
    plot_data = (
        by_card_seg.with_columns(
            pl.when(pl.col("qr_share_pct") > 0).then(pl.lit("Adoptante (>0% QR)"))
              .otherwise(pl.lit("No Adoptante (0% QR)")).alias("tipo_adopcion")
        )
...
El análisis del perfil de usuario revela tres hallazgos principales:

1.  **Distribución de Uso:** La actividad semanal por tarjeta está fuertemente concentrada en usuarios de baja frecuencia. La mediana es de solo **4 viajes por semana**, y el 95% de los usuarios realiza 13 viajes o menos. Esto justifica la segmentación en **Ocasionales (1-5)**, **Regulares (6-15)**, **Frecuentes (16-30)** e **Intensivos (31+)**, que captura eficazmente la estructura de la población.

2.  **Adopción Bimodal:** El uso de QR es un comportamiento polarizado. Los usuarios tienden a usar **casi exclusivamente un método de pago** durante la semana: o 100% tarjeta física (0% QR) o 100% QR. El uso mixto es prácticamente inexistente, lo que sugiere que la adopción tecnológica es un "switch" de comportamiento más que un proceso gradual.

3.  **Adopción por Segmento:** La inercia hacia la tarjeta física es la norma en todos los segmentos (mediana de adopción QR = 0%). Sin embargo, la proporción de usuarios que han adoptado el QR **crece con la intensidad de uso**. El segmento **"Intensivo" (31+)** es el que tiene la mayor proporción de adoptantes, indicando que los usuarios que más dependen del sistema son los que lideran la migración al pago digital.


#### Análisis Adicional de Segmentación
...
El K-Means ha validado nuestra segmentación manual, pero desde un ángulo diferente. Nos dice que la población se divide naturalmente en tres grandes grupos:
1.  **Mayoría Ocasional No Adoptante (Cluster 2):** Viajan poco, usan tarjeta.
2.  **Minoría Regular No Adoptante (Cluster 0):** Viajan mucho, usan tarjeta. Son el grupo clave para entender la inercia en usuarios frecuentes.
3.  **Grupo de Adoptantes Totales (Cluster 1):** Han migrado 100% a QR, y su frecuencia de uso es variada, pero tiende a ser media-baja.
...
if {"id_tarjeta", "wday", "hour"} <= set(lfk.collect_schema().names()):
    ...
    print("Faltan columnas 'wday' o 'hour' para la segmentación por comportamiento.")

```

#### Análisis de la Segmentación por Comportamiento

La segmentación basada en patrones de viaje (rutina y horario) revela una conexión clara entre el tipo de uso del sistema y la adopción de tecnología QR:

1.  **Composición de Segmentos:** La mayoría de los usuarios (~2.8M) exhibe un patrón de viaje **"Esporádico / Otro"**. Sin embargo, la metodología logra aislar con éxito a dos grupos clave con comportamientos definidos: los **"Usuarios de Alta Rutina"** (~455k), que viajan frecuentemente siguiendo un patrón de horas punta, y los **"Usuarios de Fin de Semana"** (~290k), que concentran su actividad en Sábado y Domingo.

2.  **Adopción y Rutina:** El hallazgo más significativo es la relación inversa entre la rutina y la adopción de QR.
    *   Los **"Usuarios de Fin de Semana"** muestran la **mayor adopción media de QR (25.2%)**. Esto sugiere que los viajes menos estructurados, probablemente ligados al ocio o actividades no laborales, fomentan el uso de herramientas digitales más flexibles.
    *   Por el contrario, los **"Usuarios de Alta Rutina"** presentan la **menor adopción media (20.0%)**. Este grupo, a pesar de su alta dependencia del sistema, muestra la mayor inercia, prefiriendo el método de pago tradicional (tarjeta física) para sus trayectos predecibles.

**Conclusión para la Tesis:** Estos resultados proporcionan una evidencia cuantitativa robusta de que **la inercia en la elección de tecnología de pago está fuertemente correlacionada con la regularidad del patrón de viaje**. La rutina fomenta el hábito y la resistencia al cambio, mientras que los contextos de viaje más variados (fin de semana) se asocian con una mayor apertura a la adopción tecnológica.
